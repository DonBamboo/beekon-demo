...
1. All auth.users data → auth_users_export.sql
2. All llm_analysis_results data → llm_analysis_results_export.sql
3. Any other missing data → missing_data_export.sql
        `);
        process.exit(1);
    }
...
`;
            ...
            for (const user of users.users) {
                ...
                const values = [
                    ...
                    user.email_change_token_new ? `'${user.email_change_token_new}'` : 'NULL', // email_change_token_new
                    ...
                    user.raw_app_meta_data ? `'${JSON.stringify(user.raw_app_meta_data).replace(/'/g, "''")}'` : 'NULL',   // raw_app_meta_data
                    user.raw_user_meta_data ? `'${JSON.stringify(user.raw_user_meta_data).replace(/'/g, "''")}'` : 'NULL', // raw_user_meta_data
                    ...
                    user.email_change_token_current ? `'${user.email_change_token_current}'` : `''`,  // email_change_token_current (default empty)
                    ...
                    user.reauthentication_token ? `'${user.reauthentication_token}'` : 'NULL', // reauthentication_token
                    user.reauthentication_sent_at ? `'${user.reauthentication_sent_at}'` : 'NULL', // reauthentication_sent_at
            ...
            console.log('Auth users exported to auth_users_export.sql');
        }

        // Export llm_analysis_results data
        console.log('Exporting llm_analysis_results data...');
...
`;
            ...
            console.log('Analysis results exported to llm_analysis_results_export.sql');
        }
