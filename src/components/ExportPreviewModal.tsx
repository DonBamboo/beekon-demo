...
═══════════════
${Array.isArray(data) && data.length > 0 ? 
  `Headers: ${Object.keys(data[0]).slice(0, 5).join(' | ')}${Object.keys(data[0]).length > 5 ? '...' : ''}
Data rows: ${data.length} records with professional formatting
Auto-sized columns with header styling` :
  'Key-value pairs with professional formatting'
}

💡 The actual Excel file will have:
• Multiple worksheets
• Formatted headers
• Auto-sized columns
