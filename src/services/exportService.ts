...
export class ExportService {
  ...
  private generateExcelExport(data: ExportData): Blob {
    // Professional Excel-compatible CSV format with BOM for UTF-8 support
    const BOM = '\uFEFF'; // Byte Order Mark for UTF-8
    let excelContent = BOM + `sep=,\n`; // Excel separator hint
    
    // Add professional title and metadata
    excelContent += `"${data.title}"\n`;
    excelContent += `"Generated by","Beekon AI"\n`;
    excelContent += `"Exported at","${new Date(data.exportedAt).toLocaleString()}"\n`;
    excelContent += `"Total Records","${data.totalRecords.toLocaleString()}"\n`;
    ...
    if (data.dateRange) {
      ...
      excelContent += `"Date Range","${startDate} to ${endDate}"\n`;
    }
    ...
    if (Array.isArray(data.data)) {
      excelContent += this.formatArrayToExcel(data.data, data.dataType);
    } else if (typeof data.data === 'object') {
      excelContent += this.formatObjectToExcel(data.data, data.dataType);
    }
  ...
  private formatArrayToExcel(data: Record<string, unknown>[], dataType?: string): string {
    ...
    let excelContent = headers.map(header => `"${header}"`).join(",") + "\n";
    ...
    processedData.forEach(row => {
      const values = headers.map(header => {
        ...
        // String values - Excel format with proper escaping
  ...
  private formatObjectToExcel(data: Record<string, unknown>, dataType?: string): string {
    ...
    Object.entries(data).forEach(([key, value]) => {
      ...
      excelContent += `"${displayName}","${formattedValue.replace(/"/g, '""')}"\n`;
    });
