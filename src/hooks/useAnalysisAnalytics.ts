...
export function useAnalysisAnalytics() {
  ...
  const transformFilters = useCallback((appFilters: AppStateFilters): ServiceFilters => {
  ...
  const stableFilters = useMemo(() => transformFilters(filters as AppStateFilters), [filters, transformFilters]);
  ...
  const invalidateAnalyticsCache = useCallback(() => {
    ...
    const analyticsKeys = allKeys.filter(key => key.startsWith(`analysis_analytics_${selectedWebsiteId}_`));
