...
export function usePrefetching(config: Partial<PrefetchConfig> = {}) {
  ...
  const loadNavigationPatterns = useCallback(() => {
    ...
    history.forEach((nav: Record<string, unknown>) => {
      ...
      if (existing) {
        ...
        existing.lastSeen = Math.max(existing.lastSeen, nav.timestamp);
      } else {
        patterns.set(key, {
          from: nav.from,
          to: nav.to,
          ...
          lastSeen: nav.timestamp,
          userAction: nav.userAction || 'direct',
        });
      }
    });
  ...
  const predictNextPages = useCallback((): PrefetchTarget[] => {
    ...
    const totalFromCurrent = fromCurrentPage.reduce((sum, p) => sum + p.frequency, 0);
  ...
  const schedulePrefetches = useCallback((targets: PrefetchTarget[]) => {
    if (!finalConfig.enabled || targets.length === 0) return;

    const systemHealth = getSystemHealth();
    
    // Reduce aggressiveness if system is under load
    const maxConcurrent = systemHealth.activeRequests > 5 
  ...
  const trackInteraction = useCallback((
    ...
    if (action === 'hover') {
      ...
      if (target && target.probability > 0.6 && !activePrefetches.current.has(key)) {
  ...
  useEffect(() => {
    ...
    if (predictions.length > 0) {
      ...
      const prefetchDelay = finalConfig.aggressiveness === 'aggressive' ? 100 : 
                           finalConfig.aggressiveness === 'moderate' ? 500 : 1000;
      
      setTimeout(() => {
        schedulePrefetches(predictions);
      }, prefetchDelay);
    }
  }, [currentPage, selectedWebsiteId, loadNavigationPatterns, predictNextPages, schedulePrefetches]);
...
export function usePrefetchTriggers() {
  ...
  const onLinkHover = useCallback((targetPage: string) => {
    trackInteraction('hover', targetPage);
  }, [trackInteraction]);
  
  const onLinkClick = useCallback((targetPage: string) => {
